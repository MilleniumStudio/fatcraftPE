                        ...
                        case "tp":
                            if($sender->hasPermission("worlds.admin.teleport")) {
								if(count($args) == 2 || count($args) == 3) {
									...
									if (count($args) == 3)
									{
										$player = $this->getServer()->getPlayer($args[$index]);
									...
									if(!$this->getServer()->isLevelLoaded($args[$index])) {
										if($this->getServer()->loadLevel($args[$index])) {

											if ($level = $this->getServer()->getLevelByName($args[$index])) {
											}
											else {
												if (isset($player)) {
													$player->sendMessage($this->getMessage("noworld"));
												}
												else {
													$sender->sendMessage($this->getMessage("noworld"));
												}
									...
									$world = $this->getServer()->getLevelByName($args[$index]);
									if (isset($player)) {
										$player->teleport($world->getSafeSpawn());
									} else {
										$sender->teleport($world->getSafeSpawn());
										$sender->sendMessage($this->getMessage("teleported", array("world" => $args[$index])));
									}
								} else {
									return false;
								}
