...
//        $this->getServer()->getScheduler()->scheduleRepeatingTask(new BWGameSender($this), 20); //todo check
    ...
    public function getAvailableTeams()
    {
        ...
        $playerPerTeam = (int)$config->get("PlayersPerTeam");
        ...
        foreach ($teams as $team) {
            ...
            foreach ($players as $player) {
                ...
                if ($p != null) {
                    $playerNametag = $p->getNameTag();
                    if ($this->getTeam($playerNametag) === $team) {
                        $teamcount++;
                    }
                }
            }
            if ($teamcount < $playerPerTeam) {
                $availableTeams[] = $team;
            }
            $teamcount = 0;
        }
        ...
        foreach ($availableTeams as $team) {
            ...
            if (count($array) == 0) {
                ...
                foreach ($players as $player) {
                    ...
                    if ($p != null) {
                        $playerNametag = $p->getNameTag();
                        if ($this->getTeam($playerNametag) === $array[0]) {
                            $teamcount2++;
                        }
                    }
                }
    ...
    public function createArena($teams, $playerPerTeam)
    {
        //todo useless ?
        $config = new Config($this->getDataFolder() . "config.yml", Config::YAML);
        ...
        $config->set("PlayersPerTeam", $playerPerTeam);
        $config->save();

        $this->getLogger()->info(TextFormat::GREEN . "Arena ready");
    }

    public function setSpawn($team, Player $p)
    {
        $config = new Config($this->getDataFolder() . "config.yml", Config::YAML);
    ...
    public function teleportToTeamSpawn(Player $player, $team)
    {
        $player->setHealth(20);
        $player->setFood(20);
        ...
        if ($player->getLevel() != $this->getServer()->getLevelByName($welt)) {
            $player->teleport($this->getServer()->getLevelByName($welt)->getSafeSpawn(), 0, 0);
        }
        $player->teleport(new Vector3($x, $y, $z), $yaw, $pitch);
    }
    ...
    public function openShop(Player $player)
    {
        ...
        $tile = Tile::createTile("Chest", $player->getLevel()->getChunk($player->getX() >> 4, $player->getZ() >> 4), $nbt); //todo check
...
//        $villager = Entity::createEntity("Villager", $level->getChunk($x >> 4, $y >> 4), $nbt);
    ...
    public function setTeamSelectionItems(Player $player)
    {
        $player->getInventory()->clearAll();
    ...
    public function sendIngameScoreboard(Player $p, $arena)
    {
        ...
        if (count($teams) >= 4) {
            foreach ($teams as $team) {
                ...
                if (in_array($team, $this->getAliveTeams())) {
...
//                    $arena = $this->getArena($sender);
                    ...
                    $sender->sendMessage(TextFormat::RED . "Vous n'êtes pas dans une arène!");
                }
            }
        }
        if ($cmd->getName() == "Bedwars" && $sender->isOP()) {
            if (!empty($args[0])) {
                if (strtolower($args[0]) == "help" && $sender->isOP()) {
                    ...
                    $sender->sendMessage(TextFormat::GRAY . "-> " . TextFormat::DARK_AQUA . "/bw help " . TextFormat::GRAY . "[" . TextFormat::RED . "Affiche tous les commandes Bedwars" . TextFormat::GRAY . "]");
                    $sender->sendMessage(TextFormat::GRAY . "-> " . TextFormat::DARK_AQUA . "/bw regsign <Arena> " . TextFormat::GRAY . "[" . TextFormat::RED . "Rejoignez un bouclier arenace" . TextFormat::GRAY . "]");
                    $sender->sendMessage(TextFormat::GRAY . "-> " . TextFormat::DARK_AQUA . "/bw savemaps <Arena> " . TextFormat::GRAY . "[" . TextFormat::RED . "Assure tous les mondes d'une arène" . TextFormat::GRAY . "]");
                    ...
                    $sender->sendMessage(TextFormat::GRAY . "-> " . TextFormat::DARK_AQUA . "/bw setlobby <Arena>" . TextFormat::GRAY . "[" . TextFormat::RED . "le lobby de l'arène" . TextFormat::GRAY . "]");
                    $sender->sendMessage(TextFormat::GRAY . "-> " . TextFormat::DARK_AQUA . "/bw setspawn <Arena> <Team>" . TextFormat::GRAY . "[" . TextFormat::RED . "set le spawn de l'équipe" . TextFormat::GRAY . "]");
                    $sender->sendMessage(TextFormat::GRAY . "-> " . TextFormat::DARK_AQUA . "/bw setbed <Arena> <Team>" . TextFormat::GRAY . "[" . TextFormat::RED . "set le lit de la team" . TextFormat::GRAY . "]");
                    $sender->sendMessage(TextFormat::GRAY . "===============");
                } elseif (strtolower($args[0]) == "regsign" && $sender->isOP()) {
                    if (!empty($args[1])) {
                        $arena = $args[1];
                        if (true/*$this->arenaExists($arena)*/) {
                            $this->registerSign = true;
                            $this->registerSignWHO = $name;
                            $this->registerSignArena = $arena;
                            $sender->sendMessage(TextFormat::GREEN . "Inscrivez maintenant un panneau!");
                        } else {
                            $sender->sendMessage(TextFormat::RED . "L'arène n'existe pas!");
                        }
                    } else {
                        $sender->sendMessage(TextFormat::RED . "/bw regsign <ArenaName>");
                    }
                } elseif (strtolower($args[0]) == "savemaps" && $sender->isOP()) {
                    $sender->sendMessage("Nope, pas implémenté");
//                    if (!empty($args[1])) {
//                        $arena = $args[1];
//                        if ($this->arenaExists($arena)) {
//                            $this->saveMaps($arena);
//                            $sender->sendMessage(TextFormat::GREEN . "Vous avez sauvegarder les cartes de l'arène " . TextFormat::AQUA . $arena . TextFormat::GREEN . " avec succès!");
//                        } else {
//                            $sender->sendMessage(TextFormat::RED . "L'arène n'existe pas !");
//                        }
//                    } else {
                    ...
                    if (!empty($args[1]) && !empty($args[2])) {
                        ...
                        if (true/*$this->arenaExists($arena)*/) {
                            if (in_array($team, $this->getAllTeams())) {
                                ...
                                $sender->sendMessage(TextFormat::GREEN . "Maintenant tapez un lit! tapez la moitié inférieure, sinon les bogues peuvent venir!");

//                                $this->resetArena($arena);
                            } else {
                                $allTeams = implode(" ", $this->getAllTeams());

                                $sender->sendMessage(TextFormat::RED . "la Team " . TextFormat::GOLD . $team . TextFormat::RED . " n'existe pas!");
                                $sender->sendMessage(TextFormat::RED . "Teams: " . $allTeams);
                            }
                        } else {
                            $sender->sendMessage(TextFormat::RED . "Arena existiert nicht!");
                        }
                    ...
                    if (!empty($args[1]) && !empty($args[2])) {
                        ...
                        if (true/*$this->arenaExists($arena)*/) {
                            if (in_array($team, $this->getAllTeams())) {
                                ...
                                $this->setSpawn($team, $sender);

                                $sender->sendMessage(TextFormat::GREEN . "Vous avez réussi à définir le spawn de l'équipe " . TextFormat::AQUA . $team);
                                $sender->sendMessage(TextFormat::GREEN . "Setup -> /bw help");
...
//                    $level = $this->plugin->getServer()->getLevelByName($welt);
                    ...
                    $maxplayers = $teams * $ppt;
                    $ingame = TextFormat::GREEN . "Entrer";

                    if ($status != "Lobby") {
                        $ingame = TextFormat::RED . "Ingame";
                    }
                    if (count($players) >= $maxplayers) {
                        $ingame = TextFormat::RED . "Plein";
                    }
                    if ($status == "Ende") {
...
//                $welt = $this->plugin->getFigthWorld($arena);
                ...
                $aliveTeams = $this->plugin->getAliveTeams();
                ...
                if ($status == "Lobby") {
                    ...
                    if (count($players) < $minplayers) {
                        ...
                        foreach ($players as $pn) {
                            ...
                            if ($p != null) {
                                $p->sendPopup(TextFormat::RED . "Il manque " . TextFormat::GOLD . $minplayers . TextFormat::RED . " joueurs");
                            } else {
//                                $this->plugin->removePlayerFromArena($arena, $pn);
                                $p->sendMessage("not implemented (l.1497)");
                            }
                        }
                        ...
                        if ((Time() % 20) == 0) {
                            foreach ($players as $pn) {
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if ($p != null) {
                                    $p->sendMessage(TextFormat::GOLD . $minplayers . TextFormat::RED . " D'autres joueurs manquent");
                                } else {
//                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                    $p->sendMessage("not implemented (l.1508)");
                                }
                            }
                        }
                    } else {
                        ...
                        if ($lobbytimer == 60 ||
                            ...
                            foreach ($players as $pn) {
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if ($p != null) {
                                    $p->sendMessage($this->prefix . "La partie commence dans " . $lobbytimer . " secondes");
                                }
                            }
                        }
...
//                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                    $p->sendMessage("not implemented (l.1538)");
                                }
                            }
                        }
                        if ($lobbytimer == 0) {
                            foreach ($players as $pn) {
                                ...
                                if ($p != null) {
                                    ...
                                    $this->plugin->teleportToTeamSpawn($p, $this->plugin->getTeam($p->getNameTag()));
                                } else {
//                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                    $p->sendMessage("not implemented (l.1554)");
                                }
                            }
                            /*
                    ...
                    if (count($aliveTeams) <= 1) {
                        if (count($aliveTeams) == 1) {
                            ...
                            $this->plugin->getServer()->broadcastMessage($this->prefix . "Team " . TextFormat::GOLD . $winnerteam . TextFormat::WHITE . "  a gagné !");
                        }
...
//                                $this->plugin->removePlayerFromArena($arena, $pn);
                        ...
                        if ($gametimer == 900 || $gametimer == 600 || $gametimer == 300 || $gametimer == 240 || $gametimer == 180) {
                            foreach ($players as $pn) {
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if ($p != null) {
                                    $p->sendMessage($this->plugin->prefix . $gametimer / 60 . " minutes restantes");
                                } else {
//                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                    $p->sendMessage("not implemented");
                                }
                            ...
                            foreach ($players as $pn) {
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if ($p != null) {
                                    $p->sendMessage($this->plugin->prefix . $gametimer . " secondes restantes");
                                } else {
//                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                    $p->sendMessage("not implemented");
                                }
                            ...
                            foreach ($players as $pn) {
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if ($p != null) {
                                    $p->sendMessage($this->plugin->prefix . "1 seconde restante");
                                } else {
//                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                    $p->sendMessage("not implemented");
                                }
                            ...
                            foreach ($players as $pn) {
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if ($p != null) {
                                    $p->sendMessage($this->plugin->prefix . "Mort subite !");

                                    $p->sendMessage($this->plugin->prefix . "Il n'y a pas de gagnant");
                    ...
                    if ($endtimer >= 0) {
                        ...
                        if ($endtimer == 15 ||
                            ...
                            foreach ($players as $pn) {
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if ($p != null) {
                                    $p->sendMessage($this->plugin->prefix . "l'arène redémarre dans " . $endtimer . " secondes");
                                } else {
//                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                    $p->sendMessage("not implemented");
                                }
